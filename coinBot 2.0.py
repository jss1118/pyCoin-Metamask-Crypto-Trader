...
# Contract addresses and ABIs for USDC and Uniswap
usdc_contract_address = '0xUSDCContractAddress'
usdc_contract_abi = json.loads('USDCContractABI')
uniswap_contract_address = '0xUniswapContractAddress'
uniswap_contract_abi = json.loads('UniswapContractABI')
...
def purchaseETH(amount_in_usdc):
    ...
    # Approve USDC transfer to the Uniswap contract
    approve_data = usdc_contract.encodeABI(fn_name='approve', args=[uniswap_contract_address, amount_in_usdc])
    signed_tx = Account.sign_transaction({
        'to': usdc_contract_address,
        'data': approve_data,
        'gas': 200000,
        'gasPrice': web3.toWei(50, 'gwei'),
        'nonce': web3.eth.getTransactionCount(web3.eth.accounts.privateKeyToAccount(private_key).address)
    }, private_key)
    ...
    swap_data = uniswap_contract.encodeABI(fn_name='swapExactTokensForETH',
                                           args=[amount_in_usdc, 0, [usdc_contract_address, web3.eth.WETH_ADDRESS],
                                                 web3.eth.accounts.privateKeyToAccount(private_key).address, int(web3.eth.getBlock('latest')['timestamp']) + 300])
    signed_tx = Account.sign_transaction({
    ...
    tx_hash = web3.eth.sendRawTransaction(signed_tx.rawTransaction)
    web3.eth.waitForTransactionReceipt(tx_hash)
    print('USDC swapped for ETH successfully!')
...
def sellETH(amount_in_eth):
    ...
    # Perform the swap on Uniswap
    uniswap_contract = web3.eth.contract(address=uniswap_contract_address, abi=uniswap_contract_abi)
    swap_data = uniswap_contract.encodeABI(fn_name='swapExactETHForTokens',
                                           args=[0, [web3.eth.WETH_ADDRESS, usdc_contract_address],
                                                 web3.eth.accounts.privateKeyToAccount(private_key).address, int(web3.eth.getBlock('latest')['timestamp']) + 300])
    signed_tx = Account.sign_transaction({
        'to': uniswap_contract_address,
        'value': amount_in_eth,
    ...
    tx_hash = web3.eth.sendRawTransaction(signed_tx.rawTransaction)
    web3.eth.waitForTransactionReceipt(tx_hash)
    print('ETH swapped for USDC successfully!')
